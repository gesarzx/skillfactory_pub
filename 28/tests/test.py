...
def test_autoriz_valid_email_pass(selenium):
    """TK-001 Тест авторизации с валидными значениями e-mail и пароль."""
...
def test_autoriz_invalid_email_pass(selenium, incor_email, incor_passw):
    """TК-002,003 "Проверка входа пользователя с невалидным email и паролем:
    1) невалидный e-mail и пароль 2)пусто - невалидный пароль. 3)невалидный e-mail и пусто 4) пусто-пусто"""
    page = AuthPage(selenium)
...
def test_elements_of_autoriz(selenium):
    """TК-004 Проверка блока "Авторизация" на наличие основных элементов."""
    page = AuthPage(selenium)
...
def test_forgot_password_link(selenium):
    """TК-008 Переход к форме "Восстановление пароля"."""
    page = AuthPage(selenium)
...
def test_registration_link(selenium):
    """TК-009 Переход к форме "Зарегистрироваться"."""
    page = AuthPage(selenium)
...
def test_page_logo_registration(selenium):
    """№№№№№№№№№№№№№TК-010 Проверка блока с логотипом/названием компании на странице "Регистрация"."""
    try:
...
def test_registration_valid_data(selenium):
    """TК-013 Проверка Регистрации пользователя по e-mail с валидными данными: "Имя" и "Фамилия" на кириллице."""
    page_reg = RegistrPage(selenium)
...
@pytest.mark.parametrize("valid_first_name",
                         [
                             ...
                             , (Settings.russian_generate_string) * 6
                             , (Settings.russian_generate_string) * 15
                             , (Settings.russian_generate_string) * 30
                         ],
                         ids=
                         [
                             'russ_symbols=2', 'russ_symbols=3', 'russ_symbols=6',
                             'russ_symbols=15', 'russ_symbols=30'
                         ])
def test_first_name_by_valid_data(selenium, valid_first_name):
    ...
    буквы кириллицы в количестве  2 ; 3 ; 6 ; 15, 30 ."""
    page_reg = RegistrPage(selenium)
...
@pytest.mark.parametrize("invalid_first_name",
                         ...
                         ids=
                         [
                             'russ_symbols=1', 'russ_symbols=31','russ_symbols=100', 'russ_symbols=256',
...
def test_first_name_invalid_data(selenium, invalid_first_name):
    ...
    пустое значение;
    буквы кириллицы в количестве 1 ; 31 ; 100 ; 256 ;
